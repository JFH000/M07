...
void writeStartMatrix(byte *pointerRegMatrix, byte *pointerRegCar)
{
  ...
  pointerRegMatrix[5] = B01000000; // Tercera fila
  pointerRegMatrix[4] = B01111000; // Cuarta fila
  pointerRegMatrix[3] = B00000100; // Quinta fila
  pointerRegMatrix[2] = B00000010; // Sexta fila
...
void checkMatrix(byte *pointerRegMatrix, byte *pointerRegCar, Status status, int level, int points)
{
  /* Global variables. */
  byte check1, check2;
...
void state_machine_run(byte *pointerRegMatrix, byte *pointerRegCar, byte *pointerShiftDir)
{
  ...
  switch(state)
  {
    ...
    case STATE_GO:
      ...
      delay(delaytime);
      checkMatrix(pointerRegMatrix, pointerRegCar, status, level, points);
      switch(key)
      {
        case KEY_LEFT:
          state = STATE_LEFT;
